#DO FIRST -- login to sciencebase

library(powstreams)
library(dplyr)
library(fBasics)

#subset all sites for assessment

regimeFolder <- 'explore/160428_PC_sprint'
gpp_table <- read.table(file.path(regimeFolder,'gpp_table.tsv'),sep='\t',header=T,stringsAsFactors = F)
er_table <- read.table("er_table.tsv",sep='\t',header=T,stringsAsFactors = F)
nep_calc <- gpp_table[,c(3:367)] + er_table[,c(3:367)]
nep_table <- cbind(er_table[,c(1:2)],nep_calc)


#1) read in the table
#2) take a site, and a year
#3) then reverse sort the values in doy.1 - doy.365
#4) find the datapoint where cumsum(values) ~= 0.5*sum(values, rm.na=T)

#sitex.year.y is the slice of data for one site for one year
...
met.summary <- bind_rows(lapply(1:nrow(gpp_table), function(i) { 
  ...
  ts <- data.frame(datetime=make_datetime(year, doy=1:365), met=unname(t(gpp_table[i, -c(1,2)])))
